...
promise_test(async testCase => {
  ...
  const getRequest3 = objectStore3.get('one');
  await promiseForTransaction(testCase, txn3);
  assert_equals(getRequest3.result.title, 'title2');
  db.close();
}, 'Transactions should stay in correct order, even if one calls commit.');
...
promise_test(async testCase => {
  ...
  await Promise.all([addWatcher.wait_for('error'), txnWatcher.wait_for('error', 'abort')]);
